...
const images = <HTMLElement>document.querySelector(".products__images");
const previews = <HTMLElement>document.querySelector(".products__previews");
const selectColor = <HTMLElement>document.querySelector("#select-color");
const selectSize = <HTMLElement>document.querySelector("#select-size");
...
products.forEach((item, index) => {
  const preview = <NodeListOf<HTMLElement>>(
    document.querySelectorAll(".products__preview")
  );
...
function createPreviews(item: { image: string; name: string }) {
...
function createOption(item: { name: string }) {
...
function createSizes(size: number) {
  let option = document.createElement("option");
  option.text = `${size}`;
  return option;
}
